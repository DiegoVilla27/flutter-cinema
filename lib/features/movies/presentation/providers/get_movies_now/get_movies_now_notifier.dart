...
/// A provider for managing the state of movies that are currently playing.
///
/// The `moviesNowNotifierProvider` is an `AsyncNotifierProvider` that provides an instance of `MoviesNowNotifier`
/// and its associated state, which is a `MovieResponseEntity`. The state represents the list of movies that are currently
/// playing or being shown in theaters.
final moviesNowNotifierProvider =
    AsyncNotifierProvider<MoviesNowNotifier, MovieResponseEntity>(
      () => MoviesNowNotifier(),
    );

/// A notifier responsible for fetching and managing the state of currently playing movies.
///
/// The `MoviesNowNotifier` extends `MoviesNotifierBase` and manages the state of a list of currently playing movies,
/// including handling pagination (for fetching additional pages of results) and updating the UI state accordingly.
///
/// It interacts with a use case (`GetMoviesNowUseCase`) to fetch the list of currently playing movies from an API or database.
...
class MoviesNowNotifier extends MoviesNotifierBase {
  ...
  /// It uses the `_fetchMovies` method to load the list of currently playing movies and return it wrapped in an `AsyncValue`.
  ///
  /// Returns:
  /// - A `Future<MovieResponseEntity>` containing the list of currently playing movies, which will be used to update the state of the notifier.
  @override
  ...
  ///
  /// This method fetches the movies for the current page by calling the `GetMoviesNowUseCase` use case. If the request is successful,
  ...
  Future<MovieResponseEntity> _fetchMovies() async {
    try {
      GetMoviesNowUseCase moviesNowUseCase = di<GetMoviesNowUseCase>();
      MovieResponseEntity newMovies = await moviesNowUseCase(currentPage);
