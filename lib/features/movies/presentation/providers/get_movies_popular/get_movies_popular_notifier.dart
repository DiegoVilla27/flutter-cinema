...
/// A provider for managing the state of movies that are currently popular.
///
/// The `moviesPopularNotifierProvider` is an `AsyncNotifierProvider` that provides an instance of `MoviesPopularNotifier`
...
/// A notifier responsible for fetching and managing the state of currently popular movies.
///
/// The `MoviesPopularNotifier` extends `MoviesNotifierBase` and manages the state of a list of currently popular movies,
/// including handling pagination (for fetching additional pages of results) and updating the UI state accordingly.
///
/// It interacts with a use case (`GetMoviesPopularUseCase`) to fetch the list of currently popular movies from an API or database.
...
class MoviesPopularNotifier extends MoviesNotifierBase {
  ...
  /// It uses the `_fetchMovies` method to load the list of currently popular movies and return it wrapped in an `AsyncValue`.
  ///
  /// Returns:
  /// - A `Future<MovieResponseEntity>` containing the list of currently popular movies, which will be used to update the state of the notifier.
  @override
  ...
  ///
  /// This method fetches the movies for the current page by calling the `GetMoviesPopularUseCase` use case. If the request is successful,
